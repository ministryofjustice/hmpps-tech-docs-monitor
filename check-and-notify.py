...
def build_slack_message(overdue_docs, pages_url):
  # The slack template needs to have blocks:
  # - the first block containing the message title in a text field,
  ...
  if slack_message_template := get_json(slack_template_filename):
    ...
    try:
      ...
      slack_message_template['blocks'][-1]['text']['text'] = list_text
    except Exception as e:
